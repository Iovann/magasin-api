...
describe('ProductsService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('Failed to create product');
      mockProductRepository.countByModelName.mockResolvedValue(0);
  ...
  describe('getTotalStock', () => {
    it('should return total stock count successfully', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Total stock count: 5' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while getting the total stock');
      mockProductRepository.count.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.getTotalStock()).rejects.toThrow(InternalServerErrorException);
  ...
  describe('getAllProducts', () => {
    it('should return all products successfully', async () => {
      ...
      expect(result).toEqual([mockProduct]);
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Found 1 products' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while getting all products');
      mockProductRepository.findAll.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.getAllProducts()).rejects.toThrow(InternalServerErrorException);
  ...
  describe('getProductById', () => {
    ...
    it('should return null when product not found', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Product with ID 999 not found' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while getting the product by ID');
      mockProductRepository.findById.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.getProductById('1')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('getStockByModel', () => {
    it('should return stock count for model successfully', async () => {
      ...
      expect(mockProductRepository.countByModelName).toHaveBeenCalledWith('Test Model');
      expect(result).toEqual({ count: 3 });
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Stock count for model Test Model: 3' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while getting the stock by model');
      mockProductRepository.countByModelName.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.getStockByModel('Test Model')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('sellProduct', () => {
    it('should sell product successfully when sufficient stock available', async () => {
      ...
      expect(mockProductRepository.findById).toHaveBeenCalledWith('1');
      expect(mockProductRepository.update).toHaveBeenCalledWith('1', { quantity: 5 });
    ...
    it('should throw bad request error when quantity is zero', async () => {
      const badRequestError = new BadRequestException('The quantity must be positive');
      mockErrorHandlingService.returnErrorOnBadRequest.mockImplementation(() => {
        throw badRequestError;
      });

      await expect(service.sellProduct('1', 0)).rejects.toThrow(BadRequestException);

      expect(mockErrorHandlingService.returnErrorOnBadRequest).toHaveBeenCalledWith(
    ...
    it('should throw bad request error when quantity is negative', async () => {
      ...
      await expect(service.sellProduct('1', -5)).rejects.toThrow(BadRequestException);
    ...
    it('should throw not found error when product does not exist', async () => {
      ...
      await expect(service.sellProduct('999', 5)).rejects.toThrow(NotFoundException);
    ...
    it('should throw bad request error when insufficient stock', async () => {
      const badRequestError = new BadRequestException('Insufficient stock for Test Product. Available quantity: 10');
      mockProductRepository.findById.mockResolvedValue(mockProduct);
      mockErrorHandlingService.returnErrorOnBadRequest.mockImplementation(() => {
        throw badRequestError;
      });

      await expect(service.sellProduct('1', 15)).rejects.toThrow(BadRequestException);
    ...
    it('should throw internal server error when update throws error', async () => {
      const internalError = new InternalServerErrorException('An error occurred while processing the sale');
      mockProductRepository.findById.mockResolvedValue(mockProduct);
      ...
      await expect(service.sellProduct('1', 5)).rejects.toThrow(InternalServerErrorException);
  ...
  describe('updateStock', () => {
    it('should update stock successfully when adding positive quantity', async () => {
      ...
      expect(mockProductRepository.findById).toHaveBeenCalledWith('1');
      expect(mockProductRepository.update).toHaveBeenCalledWith('1', { quantity: 15 });
      expect(result).toEqual(updatedProduct);
      expect(mockLogger.log).toHaveBeenCalledWith({
    ...
    it('should throw bad request error when quantity is negative', async () => {
      const badRequestError = new BadRequestException('Quantity cannot be negative');
      mockErrorHandlingService.returnErrorOnBadRequest.mockImplementation(() => {
        throw badRequestError;
      });
    ...
    it('should throw internal server error when update throws error', async () => {
      const internalError = new InternalServerErrorException('An error occurred while updating the stock');
      mockProductRepository.findById.mockResolvedValue(mockProduct);
      mockProductRepository.update.mockRejectedValue(new Error('Update failed'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
  ...
  describe('getProductsByModelName', () => {
    it('should return products by model name successfully', async () => {
      ...
      expect(mockProductRepository.findByModelName).toHaveBeenCalledWith('Test Model');
      expect(result).toEqual([mockProduct]);
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Found 1 products for model Test Model' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while getting the products by model name');
      mockProductRepository.findByModelName.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.getProductsByModelName('Test Model')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('getProductsByName', () => {
    it('should return products by name successfully', async () => {
      ...
      expect(mockProductRepository.findByName).toHaveBeenCalledWith('Test Product');
      expect(result).toEqual([mockProduct]);
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Found 1 products for name Test Product' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while getting the products by name');
      mockProductRepository.findByName.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.getProductsByName('Test Product')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('countProductsByModelName', () => {
    it('should count products by model name successfully', async () => {
      ...
      expect(mockProductRepository.countByModelName).toHaveBeenCalledWith('Test Model');
      expect(result).toEqual({ count: 3 });
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Count for model Test Model: 3' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while counting the products by model name');
      mockProductRepository.countByModelName.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.countProductsByModelName('Test Model')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('countProductsByName', () => {
    it('should count products by name successfully', async () => {
      ...
      expect(mockProductRepository.countByName).toHaveBeenCalledWith('Test Product');
      expect(result).toEqual({ count: 2 });
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Count for name Test Product: 2' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while counting the products by name');
      mockProductRepository.countByName.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.countProductsByName('Test Product')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('remove', () => {
    it('should remove product successfully', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Product 1 removed successfully' });
    });
    ...
    it('should throw internal server error when delete fails', async () => {
      const internalError = new InternalServerErrorException('Failed to delete product');
      mockProductRepository.findById.mockResolvedValue(mockProduct);
      mockProductRepository.delete.mockRejectedValue(new Error('Delete failed'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });
