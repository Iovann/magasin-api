...
describe('UsersService', () => {
  ...
  describe('create', () => {
    it('should create a new user successfully', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'User created successfully', id: mockUser.id });
    });

    it('should throw conflict error when email already exists', async () => {
      const conflictError = new ConflictException('A user with this email already exists');
    ...
    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('Failed to create user');
      mockUserRepository.findByEmail.mockResolvedValue(null);
  ...
  describe('findAll', () => {
    ...
    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while fetching all users');
  ...
  describe('findOne', () => {
    ...
    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while fetching the user by ID');
    ...
    it('should rethrow error when it has status property', async () => {
      ...
      expect(mockErrorHandlingService.returnErrorOnInternalServerError).not.toHaveBeenCalled();
    });
  });
  ...
  describe('findByEmail', () => {
    it('should return user when found by email', async () => {
      ...
      expect(result).toEqual(mockUser);
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Found user with email test@example.com' });
    });
    ...
    it('should return null when user not found by email', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'User with email nonexistent@example.com not found' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while fetching the user by email');
  ...
  describe('findByEmailWithPassword', () => {
    ...
    it('should return null when user not found by email with password', async () => {
      ...
      expect(mockUserRepository.findByEmailWithPassword).toHaveBeenCalledWith('nonexistent@example.com');
      expect(result).toBeNull();
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'User with email nonexistent@example.com not found' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while fetching the user by email with password');
      mockUserRepository.findByEmailWithPassword.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.findByEmailWithPassword('test@example.com')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('findByIdWithPassword', () => {
    it('should return user with password when found by id', async () => {
      ...
      expect(result).toEqual(mockUserWithPassword);
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'Found user with ID 1' });
    });
    ...
    it('should return null when user not found by id with password', async () => {
      ...
      expect(mockUserRepository.findByIdWithPassword).toHaveBeenCalledWith('999');
      expect(result).toBeNull();
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'User with ID 999 not found' });
    });

    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while fetching the user by ID with password');
      mockUserRepository.findByIdWithPassword.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.findByIdWithPassword('1')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('update', () => {
    ...
    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while updating the user');
      mockUserRepository.update.mockRejectedValue(new Error('Database error'));
      mockErrorHandlingService.returnErrorOnInternalServerError.mockImplementation(() => {
        throw internalError;
      });

      await expect(service.update('1', { isBlocked: true })).rejects.toThrow(InternalServerErrorException);
  ...
  describe('remove', () => {
    it('should remove user successfully', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith({ message: 'User 1 removed successfully' });
    });
    ...
    it('should throw internal server error when delete fails', async () => {
      const internalError = new InternalServerErrorException('Failed to delete user');
      mockUserRepository.findById.mockResolvedValue(mockUser);
  ...
  describe('setCurrentRefreshToken', () => {
    ...
    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while setting the refresh token');
      ...
      await expect(service.setCurrentRefreshToken('token', '1')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('removeRefreshToken', () => {
    ...
    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while removing the refresh token');
      ...
      await expect(service.removeRefreshToken('1')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('updatePasswordHash', () => {
    ...
    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while updating the password hash');
      ...
      await expect(service.updatePasswordHash('1', 'newHash')).rejects.toThrow(InternalServerErrorException);
  ...
  describe('getUserStats', () => {
    ...
    it('should throw internal server error when repository fails', async () => {
      const internalError = new InternalServerErrorException('An error occurred while fetching user statistics');
