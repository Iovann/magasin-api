...
describe('TemplateService', () => {
  ...
  describe('render', () => {
    ...
    it('should read the template file from the correct path', async () => {
      ...
      const expectedPath = join(__dirname, '..', '..', 'utils', 'templates', `${templateName}.ejs`);
    ...
    it('should throw an error if template file cannot be read', async () => {
      ...
      await expect(service.render(templateName, mockData)).rejects.toThrow('File not found');
      expect(fs.readFileSync).toHaveBeenCalledTimes(1);
    ...
    it('should throw an error if ejs rendering fails', async () => {
      ...
      await expect(service.render(templateName, mockData)).rejects.toThrow('EJS render error');
      expect(fs.readFileSync).toHaveBeenCalledTimes(1);
      expect(ejs.render).toHaveBeenCalledTimes(1);
    });
  });
