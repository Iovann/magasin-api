...
describe("DuckDBService", () => {
  ...
  describe("initialize", () => {
    it("should initialize DuckDB with correct parameters", async () => {
      ...
      expect(mockDuckDB.DuckDBInstance.create).toHaveBeenCalledWith(
        "/test/path/to/db.duckdb",
      );

      // Récupérer l'instance mockée pour vérifier les appels
      const mockInstanceCall =
        mockDuckDB.DuckDBInstance.create.mock.results[0].value;
    ...
    it("should not initialize twice if already initialized", async () => {
      ...
      expect(mockDuckDB.DuckDBInstance.create.mock.calls.length).toBe(
        callCount,
      );
    });
  });
