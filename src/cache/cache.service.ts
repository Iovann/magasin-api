...
export class CacheService {
   ...
   */
  async set<T>(key: string, value: T, { ttl }: { ttl?: number }): Promise<void> {
   ...
   */
  async wrap<T>(key: string, fn: () => Promise<T>, { ttl }: { ttl?: number }): Promise<T> {
   ...
   */
  async getOrSet<T>(key: string, fn: () => Promise<T>, { ttl }: { ttl?: number }): Promise<T> {
   ...
   * ], { ttl: 3600 });
   */
  async mset<T>(entries: Array<{ key: string; value: T }>, { ttl }: { ttl?: number }): Promise<void> {
    try {
