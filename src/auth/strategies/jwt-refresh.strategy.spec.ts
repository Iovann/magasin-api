...
describe('JwtRefreshStrategy', () => {
  ...
  describe('validate', () => {
    it('should return the user if refresh token matches', async () => {
      ...
      const result = await jwtRefreshStrategy.validate(mockRequest as any, payload);
    ...
    it('should throw UnauthorizedException if refresh token does not match', async () => {
      ...
      await expect(jwtRefreshStrategy.validate(mockRequest as any, payload)).rejects.toThrow(UnauthorizedException);
      expect(mockRequest.get).toHaveBeenCalledWith('authorization');
      expect(usersService.getUserIfRefreshTokenMatches).toHaveBeenCalledWith(
